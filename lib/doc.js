...
function fileToDoc(fileInfo, courseInfo, coursePageIndex) {
    ...
    if (fileInfo['file_location'] != null)
        fileName = fileInfo['file_location'].slice(fileInfo['file_location'].indexOf(fileInfo['uid'])).replace(fileInfo['uid'].concat('_'), '');
    else
    ...
    return Object.assign({ "@id": originalFileUrl, "@type": [
            "Resource",
            "Document"
        ], name: fileTitle, description: fileDescription, keywords: courseKeywords, license: mitLicense, learningResourceType: learningResourceType, inLanguage: courseLanguage }, (courseStartDate != null ? { dateCreated: courseStartDate } : {}), { educationalLevel: courseLevel, sourceOrganization: [
            "https://ocw.mit.edu"
        ], topic: courseUrl });
}
